...
def draw_nodes_found_by_minimal_steiner():
    ...
    plt.savefig('../data/outputs/nodes_found_by_minimal.png')
    plt.show()
...
def draw_costs_by_minimal_steiner():
    ...
    plt.savefig('../data/outputs/costs_of_minimal_vary_with_keywords_num.png')
...
def draw_minimal_found_nodes_between_random_and_mashup_keywords():
    ...
    plt.legend()
    plt.savefig('../data/outputs/nodes_of_minimal_between_random_and_mashup_keywords.png')
...
def draw_minimal_costs_between_random_and_mashup_keywords():
    ...
    print(len(average_costs))
    print(len(average_costs2))

    print(average_costs[0])
    print(average_costs2[0])

    plt.xlabel('l: number of keywords')
    plt.ylabel('computation time (s)')
    plt.bar(keyword_num_options - width, average_costs[0], width = width, label='mashup keywords')
    plt.bar(keyword_num_options, average_costs2[0], width=width, label='random keywords')

    plt.legend()
    plt.savefig('../data/outputs/costs_of_minimal_between_random_and_mashup_keywords.png')
...
def draw_success_rate_between_random_and_mashup_keywords():
    ...
    plt.bar(keyword_num_options + width, success_rate[1], width=width, label='random keywords')

    plt.legend(loc='upper left')#, bbox_to_anchor=(0.8, 1.2))
    plt.savefig('../data/outputs/success_rates_of_minimal_between_random_and_mashup_keywords_by2.png')
    plt.show()
...
def draw_overlap_between_all_and_head_tail_keywords():
    ...
    plt.bar(x, average_num_nodes2[0], width=width,label='Minimal Steiner')
    ...
    plt.savefig('../data/outputs/nodes_found_by_methods.png')

draw_success_rate_between_random_and_mashup_keywords()
draw_success_rates_of_different_methods()

draw_convergence_with_times()
draw_nodes_with_different_methods()
draw_costs_with_different_methods()
draw_minimal_costs_between_random_and_mashup_keywords() #no
draw_nodes_found_by_minimal_steiner()
draw_overlap_between_all_and_head_tail_keywords()
