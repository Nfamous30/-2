...
import mashup
from util import prepare_data, generateKeywords
...
def minimal_steiner_vary_with_keywords_num(times = 10):
    '''
    ...
    for (i, count) in enumerate(count_options):
        for j in range(times):
            ...
            sample_dict = mashup.sample_categories_of_mashup(24)
            keywords_list = sample_dict[count]
            keywords = keywords_list[j%24]
            print(keywords)
    ...
    with open('../data/outputs/minmal_steiner_vary_with_keywords_num.json', 'w') as f:
        json.dump(dict, f)
...
def compare_vary_with_keywords_num(times = 50):
    ...
    dict['costs'] = costs.tolist()
    #
    with open('../data/outputs/compare_vary_with_keywords_num.json', 'w') as f:
        json.dump(dict, f)

def minimal_steiner_vary_with_keywords_num_in_mashup(sample_count = 10):
    '''
    ...
    with open('../data/outputs/minmal_steiner_vary_with_keywords_num_in_mashup.json', 'w') as f:
        json.dump(dict, f)


def compare_vary_with_keywords_num_in_mashup(sample_count = 10):
    '''
    以mashup中调用的api的类别（每个api取一个category）为关键词，测试返回的节点数和时间
    对一个每一组mashup中的api类别，我们采用两种形式检索，如要检索的关键词为ABCD，我们会分别检索ABCD和AD两种形式的关键词
    查看其返回的节点数和花费时间
    ...
    with open('../data/outputs/compare_vary_with_keywords_num_in_mashup.json', 'w') as f:
        json.dump(dict, f)

def compare_head_tail_keyword_in_mashup(sample_count = 10):
    '''
    以mashup中调用的api的类别（每个api取一个category）为关键词，测试返回的节点数和时间
    对一个每一组mashup中的api类别，我们采用两种形式检索，如要检索的关键词为ABCD，我们会分别检索ABCD和AD两种形式的关键词
    查看其返回的节点数和花费时间
    ...
    with open('../data/outputs/compare_head_tail_keywords_in_mashup.json', 'w') as f:
        json.dump(dict, f)
...
# compare_head_tail_keyword_in_mashup(24) 
compare_vary_with_keywords_num_in_mashup(24)  
# minimal_steiner_vary_with_keywords_num(24)
# compare_vary_with_keywords_num(100) 
# minimal_steiner_vary_with_keywords_num_in_mashup(24)
# compare_head_tail_keyword_in_mashup(24)
